...
- **フロントエンド**: Nuxt4 + Cloudflare Pages
- **認証**: Better Auth
- **データベース**: Turso
- **決済**: Stripe
- **AI**: Cloudflare Workers AI
- **Rate Limit**: Workers KV, Better Auth
...
1. **フロントエンド**: Better Auth でログイン
2. **バックエンド**: ミドルウェアで検証 → `c.get('user')` でユーザー情報取得
3. **プラン情報**: Turso から取得（毎回確認）
...
- **Free**: 無料
- **Pro**: 年額 5000 円（月額換算約 417 円）
- **Premium**: 年額 8,000 円（月額換算約 667 円）+ 7 日間無料トライアル
...
| **AI タグ生成回数**  | 15 回/日  | 50 回/日  | 100 回/日    |
| **AI 分析回数**      | 3 回/日   | 10 回/日  | 100 回/日    |
...
- **機能**: スケジュールでタイトルとサブタイトル(任意)を入力することで自動でタグ付けを行う
- **用途**: 可視化などでどういうことを行ったのかが一目でわかる
- **ユーザー負担**: 手入力不要
...
- **ユーザー確認**: Better Auth の認証情報をミドルウェアで検証する
- **プラン情報**: TursoTurso から取得（毎回確認）
...
await db.delete(study*sessions)
.where(/* 古い順で削除 \_/)
}## コスト試算
...
1. **ゆるい連続記録**: プレッシャーにならない連続記録
2. **週間達成通知**: 承認だけ、評価はしない
3. **勉強開始の「きっかけ」記録**: 任意のメモ
